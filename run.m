...
sn_positioning = ["RMMS","MRMS-NC","MRMS-C"]; % Mobile Sink Positioning Method to be compared
% Possible values: random, even_nonconfined, even_confined
pn_select_method = ["cluster_head", "prediction"]; % cluster_head only applies to random.
...
sn_max_dist = 20; %Maximum mobility for sink nodes (in m)
min_visit_dist = 30; % Minimum distance to affirm visitation by sink nodes (in m)
...
for sn_method = sn_positioning
    for pn_method = pn_select_method
        ...
        if strcmp(pn_method, "cluster_head") && ~strcmp(sn_method, "RMMS")
            continue
        elseif strcmp(pn_method, "cluster_head") && strcmp(sn_method, "RMMS")
